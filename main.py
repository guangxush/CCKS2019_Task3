...
# 创建多折任务模型
def model_fold_select(model_name, x_train, x_train_dis1, x_train_dis2, y_train, x_valid, x_valid_dis1, x_valid_dis2,
                      y_valid,
                      x_test, x_test_dis1, x_test_dis2, level, fold, results_cv, overwrite=False):
    config = Config()
    config.level = level
    if level == 'word':
        # 固定最大长度，多余的截取掉，不足的用0填充
    ...
    if overwrite or not os.path.exists(os.path.join(config.checkpoint_dir, '%s.hdf5' % (model_name + '_' + str(fold)))):
        print('Start training the ' + model_name + ' model...')
        model.fit(x_train, x_train_dis1, x_train_dis2, y_train, x_valid, x_valid_dis1, x_valid_dis2, y_valid)
    model.load_weight()
    print('Start evaluate the ' + model_name + ' model...')
...
if __name__ == '__main__':
    ...
    if flag == 'multi':
        ...
        x_train, y_train, x_valid, y_valid, x_test, ids = get_tf_idf_data(train_file='./data/sent_train_multi.txt',
        ...
        for i in range(10):
            ...
            results_cv = model_fold_select('cnn_base', x_train, x_train_dis1, x_train_dis2, y_train, x_valid,
                                           x_valid_dis1,
                                           x_valid_dis2, y_valid,
                                           x_test, x_test_dis1, x_test_dis2, level, i, results_cv, overwrite=overwrite)
        write_results(ids_, results_cv)
