...
# 创建单一任务模型
def model_bag_select(model_name, x_train, x_train_dis1, x_train_dis2, y_train, x_valid, x_valid_dis1, x_valid_dis2, y_valid,
                 x_test, x_test_dis1, x_test_dis2, level, overwrite=False):
    config = Config()
    config.level = level
    if level == 'word':
    ...
    model.evaluate(model_name, y_valid_pred, y_valid)
    print('End generate the ' + model_name + ' model...')
...
if __name__ == '__main__':
    ...
    if flag == 'multi':
        ...
        # cnn base model
        y_test_pred = model_bag_select('han_yang', x_train, x_train_dis1, x_train_dis2, y_train, x_valid, x_valid_dis1,
                                       x_valid_dis2, y_valid,
                                       x_test, x_test_dis1, x_test_dis2, level, overwrite=overwrite)
